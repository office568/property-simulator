...
def save_to_google_sheets(name, user_email, data_dict):
    ...
    # Logic to handle missing columns or empty dataframes
...
if st.sidebar.button("💾 クラウドに保存", use_container_width=True):
    if not new_save_name:
        ...
        # Don't save internal session keys like 'user_email' or the password widgets
        to_save = {k: v for k, v in st.session_state.items() if not k.startswith("FormSubmit") and k != "user_email" and "_slider" not in k and "_num" not in k}
