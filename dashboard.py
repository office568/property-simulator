...
# --- NEW: CUSTOM COMMA INPUT FUNCTION ---
def comma_input(label, key, step=1000):
    # Get current value from session state
    val = st.session_state.get(key, 0)
    # Display the number input with forced comma formatting logic
    new_val = st.number_input(label, value=int(val), step=step, key=f"ui_{key}")
    # Update the actual key used for calculations
...
def save_to_google_sheets(name, data_dict):
    df = load_db()
    clean_dict = {k: v for k, v in data_dict.items() if not k.startswith("FormSubmit") and "password" not in k and "confirm" not in k and not k.startswith("ui_")}
...
# SECTION 1: INITIAL INVESTMENT (FORCED COMMA VIEW)
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    rent_total = comma_input("月額ローン及び家賃 (円)", "rent_total")
    ...
    st.markdown("---")
    shikikin = comma_input("敷金 (円)", "shikikin")
    reikin = comma_input("礼金 (円)", "reikin")
    broker_fee = comma_input("仲介手数料 (円)", "broker_fee")
    photo_shooting = comma_input("写真撮影 (円)", "photo")
    renovation = comma_input("リフォーム (円)", "renov", step=10000)
    furniture_appliances = comma_input("家具＋家電 (円)", "furn", step=10000)
    guarantee_fee = comma_input("保証会社費用 (円)", "guar")
    fire_insurance = comma_input("火災保険 (円)", "fire")
    license_fee = comma_input("旅館業許可 (円)", "license")
    fire_safety_work = comma_input("消防設備の工事 (円)", "fire_work")
    other_init = comma_input("その他予備費 (円)", "other")
...
for i in range(num_types):
    with st.sidebar.expander(f"部屋タイプ {i+1}", expanded=True):
        ...
        r_adr = comma_input("平均単価 ADR (円)", f"a_{i}", step=500)
        r_cons = comma_input("1日あたり消耗品 (円)", f"cons_{i}", step=10)
        r_util = comma_input("1日あたり光熱費 (円)", f"u_{i}", step=10)
...
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    ...
    fixed_op_costs = comma_input("固定管理費 (円)", "fixed_costs")
...
if st.sidebar.button("💾 クラウドに保存"):
    to_save = {k: v for k, v in st.session_state.items() if not k.startswith("FormSubmit") and "password" not in k and "confirm" not in k and not k.startswith("ui_")}
