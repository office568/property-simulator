...
# 1. Initial Investment (Startup Costs - 10 Items)
...
for i in range(int(num_types)):
    with st.sidebar.expander(f"部屋タイプ {i+1} の詳細", expanded=True):
        ...
        adr = st.number_input(f"平均単価 ADR ({name})", value=0, step=1000, format="%d", key=f"a_{i}")
        consumables = st.number_input(f"1日あたり消耗品 ({name})", value=0, step=100, format="%d", key=f"cons_{i}")
        util_day = st.number_input(f"1日あたり光熱費 ({name})", value=0, step=100, key=f"u_{i}")
        room_configs.append({"name": name, "count": count, "adr": adr, "consumables": consumables, "util_day": util_day})

# 3. Operating Parameters
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    target_occ = st.slider("全体の想定稼働率 %", 10.0, 100.0, 70.0, step=0.1)
    ota_fee_rate = st.slider("OTA手数料率 %", 0.0, 30.0, 15.0, step=0.1)
    # 追加項目：管理費用スライダー
    ...
    cape_rate = st.slider("メンテナンス (修理、CAPEX, FF&E等) %", 0.0, 10.0, 1.0, step=0.1)
...
with col_cost:
    ...
    cost_detail_df = pd.DataFrame({
        "項目": ["家賃", "消耗品", "光熱費", "固定管理費 (ソフト、Wifi等)", "メンテナンス (修理、CAPEX, FF&E等)", "管理費用", "OTA手数料", "営業利益"],
...
for o in [30, 40, 50, 60, 70, 80, 90, 100]:
    ...
    o_mgmt = o_rev * (management_fee_rate / 100) # 感度分析用の管理費計算
