...
# This section ensures that every key exists before the widgets try to access them
defaults = {
...
def save_property_to_db(name, data_dict):
    ...
    # Clean dictionary to avoid saving temporary streamlit keys
...
try:
    ...
    st.sidebar.info("💡 Logo file 'logo.png' not found.")
...
if not current_db.empty:
    ...
    if st.sidebar.button("選択した物件を読み込む"):
        ...
        # We delete all existing widget state to force a fresh pull from the newly set session state
