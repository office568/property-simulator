...
def delete_property_from_db(name):
    df = load_db()
    if not df.empty:
        df = df[df.property_name != name]
        df.to_csv(DB_FILE, index=False)
...
if not db.empty:
    ...
    if col_d.button("🗑️ 削除"):
        st.sidebar.warning(f"{target_prop} を削除しますか？")
        if st.sidebar.button("はい、削除します"):
            delete_property_from_db(target_prop)
