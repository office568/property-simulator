...
defaults = {
    ...
    "num_types": 2, "fixed_costs": 0, "target_profit": 500000, "val_prep": 2, "val_prep_num": 2,
...
# SECTION 3: OPERATING PARAMS & GOAL
with st.sidebar.expander("3. 運営コスト・稼働率・目標設定", expanded=True):
    st.write("想定稼働率 %")
    ...
    st.caption(f"固定費用: {fmt(st.session_state.fixed_costs)}")
    fixed_op_costs = st.number_input("ソフト、インターネット、その他の固定費用", step=1000, key="fixed_costs")
    
    st.markdown("---")
    st.caption(f"目標利益: {fmt(st.session_state.target_profit)}")
...
export_data = {
    "項目": ["物件名", "初期投資合計", "月間想定売上", "月間費用合計", "月間営業利益", "投資回収期間", "目標月間利益"],
    "値": [new_save_name if new_save_name else "未設定", fmt(startup_cost), fmt(total_rev), fmt(monthly_cost), fmt(profit), f"{payback:.1f}ヶ月", fmt(target_profit_val)]
}
export_df = pd.DataFrame(export_data)
csv_buffer = io.StringIO()
export_df.to_csv(csv_buffer, index=False)
st.sidebar.download_button(label="📥 CSVエクスポート", data=csv_buffer.getvalue(), file_name=f"sim_{new_save_name}.csv", mime="text/csv")
...
# 11. BREAK-EVEN & STRATEGY ANALYSIS SECTION
st.subheader("📈 戦略分析と目標単価 (Strategy & Goal Analysis)")
col_be1, col_be2 = st.columns([1, 2])

with col_be1:
    st.info("**基本ユニット分析 / Unit Analysis**")
    st.metric(label="平均変動費 / 泊", value=fmt(avg_var_per_night), help="[EN] Costs incurred only when a guest stays. [JP] 宿泊時のみ発生する費用（消耗品、光熱費など）。")
    st.metric(label="固定費合計 / 月", value=fmt(fixed_monthly), help="[EN] Monthly costs regardless of occupancy. [JP] 稼働に関わらず毎月支払う費用（家賃、固定費）。")
    cm_ratio = (1 - total_var_rate) * 100
    st.metric(label="貢献利益率", value=f"{cm_ratio:.1f}%", help="""
    [EN] This shows how much of every ¥100 earned is left to pay your rent after OTA/Mgmt fees. Higher is better!
    [JP] 売上100円につき、手数料などを払った後に「手元に残る金額」です。これが高いほど効率が良いビジネスです。
    """)

with col_be2:
    st.write(f"**目標月間利益 {fmt(target_profit_val)} を達成するための必要ADR**")
    be_rows = []
    for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
        occ_nights = num_rooms * days * (occ_p / 100)
        if occ_nights > 0:
            # Formula to reach Target Profit: ((Fixed_Costs + Target_Profit) / Nights + Var_Cost_per_Night) / (1 - Var_Rate)
            target_adr = ((fixed_monthly + target_profit_val) / occ_nights + avg_var_per_night) / (1 - total_var_rate)
            
            # Strategy Logic
            if occ_p <= 40: strategy = "閑散期：赤字回避のため高単価が必須です。"
            elif occ_p <= 70: strategy = "通常期：単価とレビューのバランスを重視。"
            else: strategy = "繁忙期：少し単価を下げても確実に利益が出ます。"
            
            be_rows.append({"稼働率": f"{occ_p}%", "必要ADR": target_adr, "Your Strategy": strategy})
    
    be_df = pd.DataFrame(be_rows)
    st.table(be_df.style.format({"必要ADR": "¥{:,.0f}"}))
