...
# 4. CALLBACK FUNCTIONS FOR PERFECT SYNC
def sync_slider(key_from, key_to):
    st.session_state[key_to] = st.session_state[key_from]

# Initialize Session States for Synced Widgets if not present
...
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    ...
    st.write("開業準備期間 (ヶ月)")
    prep_months = st.slider("Slider", 0, 6, value=2, key="prep_slider", label_visibility="collapsed")
...
# Section 3: Operating Parameters (WITH BIDIRECTIONAL SYNC)
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    
    # --- Occupancy Sync ---
    st.write("全体の想定稼働率 %")
    c_occ1, c_occ2 = st.columns([2, 1])
    target_occ = c_occ1.slider("Slider_Occ", 10.0, 100.0, step=0.1, key="val_occ", label_visibility="collapsed")
    target_occ_input = c_occ2.number_input("Num_Occ", 10.0, 100.0, step=0.1, key="val_occ_num", label_visibility="collapsed", 
                                           value=st.session_state.val_occ, on_change=sync_slider, args=("val_occ_num", "val_occ"))
    # Manual back-sync for the slider
    st.session_state.val_occ_num = target_occ

    # --- OTA Fee Sync ---
    st.write("OTA手数料率 %")
    c_ota1, c_ota2 = st.columns([2, 1])
    ota_fee_rate = c_ota1.slider("Slider_OTA", 0.0, 30.0, step=0.1, key="val_ota", label_visibility="collapsed")
    ota_fee_input = c_ota2.number_input("Num_OTA", 0.0, 30.0, step=0.1, key="val_ota_num", label_visibility="collapsed",
                                        value=st.session_state.val_ota, on_change=sync_slider, args=("val_ota_num", "val_ota"))
    st.session_state.val_ota_num = ota_fee_rate

    # --- Management Fee Sync ---
    st.write("管理費用 %")
    c_mgmt1, c_mgmt2 = st.columns([2, 1])
    management_fee_rate = c_mgmt1.slider("Slider_Mgmt", 0.0, 40.0, step=0.5, key="val_mgmt", label_visibility="collapsed")
    mgmt_fee_input = c_mgmt2.number_input("Num_Mgmt", 0.0, 40.0, step=0.5, key="val_mgmt_num", label_visibility="collapsed",
                                          value=st.session_state.val_mgmt, on_change=sync_slider, args=("val_mgmt_num", "val_mgmt"))
    st.session_state.val_mgmt_num = management_fee_rate
    ...
    cape_rate = c_cape1.slider("Slider_Cape", 0.0, 10.0, step=0.5, key="val_cape", label_visibility="collapsed")
    cape_input = c_cape2.number_input("Num_Cape", 0.0, 10.0, step=0.5, key="val_cape_num", label_visibility="collapsed",
                                      value=st.session_state.val_cape, on_change=sync_slider, args=("val_cape_num", "val_cape"))
    st.session_state.val_cape_num = cape_rate
