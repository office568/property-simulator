...
# SECTION 1: INITIAL INVESTMENT (No Decimals)
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    rent_total = st.number_input("月額ローン及び家賃 (円)", step=1000, format="%d", key="rent_total")
    st.write("開業準備期間 (ヶ月)")
    c_p1, c_p2 = st.columns([2, 1])
    prep_months = c_p1.slider("Slider Prep", 0, 6, key="val_prep", on_change=update_prep_num, label_visibility="collapsed")
    prep_months_num = c_p2.number_input("Num Prep", 0, 6, step=1, format="%d", key="val_prep_num", on_change=update_prep_slider, label_visibility="collapsed")
    prep_rent_cost = rent_total * prep_months
    
    st.markdown("---")
    shikikin = st.number_input("敷金 (円)", step=1000, format="%d", key="shikikin")
    reikin = st.number_input("礼金 (円)", step=1000, format="%d", key="reikin")
    broker_fee = st.number_input("仲介手数料 (円)", step=1000, format="%d", key="broker_fee")
    photo_shooting = st.number_input("写真撮影 (円)", step=1000, format="%d", key="photo")
    renovation = st.number_input("リフォーム (円)", step=1000, format="%d", key="renov")
    furniture_appliances = st.number_input("家具＋家電 (円)", step=1000, format="%d", key="furn")
    guarantee_fee = st.number_input("保証会社費用 (円)", step=1000, format="%d", key="guar")
    fire_insurance = st.number_input("火災保険 (円)", step=1000, format="%d", key="fire")
    license_fee = st.number_input("旅館業許可 (円)", step=1000, format="%d", key="license")
    fire_safety_work = st.number_input("消防設備の工事 (円)", step=1000, format="%d", key="fire_work")
    other_init = st.number_input("その他予備費 (円)", step=1000, format="%d", key="other")

# SECTION 2: ROOM CONFIG (Clean Labels & No Decimals)
st.sidebar.markdown("### 2. 部屋タイプ別の設定")
num_types = int(st.sidebar.number_input("部屋タイプの種類数", min_value=1, max_value=5, step=1, format="%d", key="num_types"))
room_configs = []
for i in range(num_types):
    with st.sidebar.expander(f"部屋タイプ {i+1}", expanded=True):
        r_name = st.text_input("部屋タイプ名", key=f"name_{i}")
        r_count = st.number_input("部屋数", min_value=1, step=1, format="%d", key=f"c_{i}")
        r_adr = st.number_input("平均単価 ADR (円)", step=100, format="%d", key=f"a_{i}")
        r_cons = st.number_input("1日あたり消耗品 (円)", step=10, format="%d", key=f"cons_{i}")
        r_util = st.number_input("1日あたり光熱費 (円)", step=10, format="%d", key=f"u_{i}")
        room_configs.append({"name": r_name, "count": r_count, "adr": r_adr, "consumables": r_cons, "util_day": r_util})

# SECTION 3: OPERATING PARAMS (Decimals kept for percentages)
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    ...
    fixed_op_costs = st.number_input("固定管理費 (円)", step=1000, format="%d", key="fixed_costs")
