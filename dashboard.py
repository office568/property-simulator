...
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    ...
    val_prep = st.number_input("開業準備期間 (ヶ月) ⓘ", min_value=0, max_value=12, key="val_prep", help="家賃のみ発生する期間です。")
    ...
    # 追加：保険会社費用
    guar_fee = st.number_input("保険会社費用 (円)", min_value=0, step=1000, key="guar", help="家賃保証会社費用や火災保険料などです。")
...
# 安全な貢献利益率計算
cm_ratio = (1 - total_fees_rate) * 100
if pd.isna(cm_ratio) or cm_ratio == float('inf'): cm_ratio = 0
...
# 初期投資合計（保険会社費用を含める）
startup_total = prep_rent_cost + shikikin + reikin + broker_fee + guar_fee + renovation + furniture_appliances + license_fee + fire_safety_work + other_init
payback = startup_total / profit if profit > 0 else 0

# --- 9. ダッシュボード表示 ---
st.subheader("📌 収支シミュレーション結果")
m1, m2, m3, m4, m5 = st.columns(5)
m1.metric("初期投資合計", fmt(startup_total), help="保険会社費用等を含む開業費用の総額です。")
...
with cl:
    ...
    # 保険項目をグラフに追加
    df_i = pd.DataFrame({
        "項目": ["空家賃","敷金","礼金","仲介料","保険","リフォーム","家具家電","許可","工事","他"],
        "金額": [prep_rent_cost, shikikin, reikin, broker_fee, guar_fee, renovation, furniture_appliances, license_fee, fire_safety_work, other_init]
    })
...
with col_a:
    ...
    st.metric(label="貢献利益率", value=f"{int(round(cm_ratio))}%", help="売上から手数料を引いた後、固定費の支払いに回せる利益の割合です。")

with col_b:
    st.write("**稼働率別の必要ADR目安**")
    be_data = []
    tot_rooms = sum(r['count'] for r in room_configs)
    fee_denom = (1 - total_fees_rate)
    ...
    for o in [30, 50, 70, 90]:
        ...
        if nights > 0 and fee_denom > 0:
            ...
            target_adr = ((fixed_all + target_profit_val) / nights + var_night) / fee_denom
            be_data.append({"稼働率": f"{o}%", "目標達成ADR": f"¥{int(target_adr):,}"})
        else:
            be_data.append({"稼働率": f"{o}%", "目標達成ADR": "計算不可"})
    st.table(be_data)
