...
# SECTION 4: GOAL SETTING (Independent Section)
...
startup_cost = (prep_rent_cost + st.session_state.shikikin + st.session_state.reikin + st.session_state.broker_fee + renovation + furniture_appliances + license_fee + fire_safety_work + other_init)
...
# 11. STRATEGY ANALYSIS SECTION (Simplified & Independent Analysis)
...
with col_be1:
    ...
    [JP] 売上100円につき、手数料などを払った後に「家賃などの固定費の支払いに回せる金額」です。これが高いほど、効率よく利益が出るビジネスと言えます。
    """)
...
with col_be2:
    ...
    for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
        ...
        if occ_nights > 0:
            # Formula for Break-even ADR (Profit ¥0)
            be_adr = ((fixed_monthly / occ_nights) + avg_var_per_night) / (1 - total_var_rate)
            # Formula for Target ADR (Profit ¥Target)
            target_adr = ((fixed_monthly + target_profit_val) / occ_nights + avg_var_per_night) / (1 - total_var_rate)
            
            # Use "Markup" instead of "Strategy" text
            markup = target_adr - be_adr
            
            be_rows.append({
                "稼働率": f"{occ_p}%", 
                "損益分岐ADR (利益¥0)": be_adr, 
                "目標達成ADR (利益¥目標)": target_adr,
                "利益上乗せ額": markup
            })
    
    be_df = pd.DataFrame(be_rows)
    st.table(be_df.style.format({
        "損益分岐ADR (利益¥0)": "¥{:,.0f}",
        "目標達成ADR (利益¥目標)": "¥{:,.0f}",
        "利益上乗せ額": "¥{:,.0f}"
    }))
