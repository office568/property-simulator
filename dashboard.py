...
# Uses credentials from st.secrets to connect to your permanent database
conn = st.connection("gsheets", type=GSheetsConnection)

def load_db():
    try:
        # ttl="0s" forces a fresh read from Google Sheets every time
        df = conn.read(ttl="0s")
...
def save_to_google_sheets(name, data_dict):
    df = load_db()
    # Filter out internal keys and confirm states
    ...
    # Overwrite if property name already exists
...
# Pre-initialize dynamic room keys
for i in range(5):
...
if not db.empty:
    ...
    if col_load.button("📥 読み込む"):
        ...
        # Sync the shadow state for sliders
    ...
    if st.session_state.confirm_delete:
        st.sidebar.error(f"本当に {target_prop} を削除します敢？")
