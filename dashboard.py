...
# 11. STRATEGY ANALYSIS SECTION (Simplified as requested)
...
with col_be1:
    ...
    st.metric(label="平均変動費 / 泊", value=fmt(round(avg_var_per_night)), help="[EN] Costs incurred only when a guest stays. [JP] 宿泊時のみ発生する費用。")
    st.metric(label="固定費合計 / 月", value=fmt(round(fixed_monthly)), help="[EN] Monthly costs regardless of occupancy. [JP] 稼働に関わらず毎月支払う費用。")
    
    cm_ratio = (1 - total_var_rate) * 100
    st.metric(label="貢献利益率", value=f"{round(cm_ratio)}%", help="[EN] Portion of sales to pay rent after fees. [JP] 売上のうち、固定費の支払いに回せる利益の割合。")
...
with col_be2:
    ...
    for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
        ...
        if occ_nights > 0:
            ...
            be_rows.append({
                "稼働率": f"{occ_p}%", 
                "損益分岐ADR (利益¥0)": round(be_adr), 
                "目標達成ADR (利益¥目標)": round(target_adr)
            })
    
    be_df = pd.DataFrame(be_rows)
    # Format the table numbers with Yen sign and commas, no decimals
