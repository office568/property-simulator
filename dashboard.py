...
# 11. STRATEGY ANALYSIS SECTION (Option 2 Replacement)
...
with col_be1:
    ...
    st.metric(label="平均変動費 / 泊", value=fmt(avg_var_per_night), help="[EN] Costs incurred only when a guest stays. [JP] 宿泊時のみ発生する費用。")
    st.metric(label="固定費合計 / 月", value=fmt(fixed_monthly), help="[EN] Monthly costs regardless of occupancy. [JP] 稼働に関わらず毎月支払う費用。")
    
    cm_ratio = (1 - total_var_rate) * 100
    st.metric(label="貢献利益率", value=f"{cm_ratio:.1f}%", help="[EN] Portion of sales to pay rent after fees. [JP] 売上のうち、固定費の支払いに回せる利益の割合。")
    
    # Safety Margin Tooltip Explanation
    st.metric(
        label="安全余裕率 (平均値)", 
        value=f"{((profit / total_rev) * 100) if total_rev > 0 else 0:.1f}%",
        help="""
        [EN] Safety Margin %: The percentage of your target price that is actual profit after all bills are paid. Higher is safer.
        [JP] 安全余裕率: 目標単価のうち、すべての支払いを終えた後に「利益」として残る割合。高いほどリスクに強い経営と言えます。
        """
    )

with col_be2:
    st.write(f"**稼働率別：損益分岐点と安全余裕率**")
    be_rows = []
    for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
        ...
        if occ_nights > 0:
            ...
            # Target ADR to hit target profit
            target_adr = ((fixed_monthly + target_profit_val) / occ_nights + avg_var_per_night) / (1 - total_var_rate)
            # Safety Margin % = (Target ADR - Break-even ADR) / Target ADR
            safety_margin = ((target_adr - be_adr) / target_adr) * 100 if target_adr > 0 else 0
            
            be_rows.append({
                "稼働率": f"{occ_p}%", 
                "損益分岐ADR (利益¥0)": be_adr, 
                "目標達成ADR (利益¥目標)": target_adr,
                "安全余裕率 (%)": f"{safety_margin:.1f}%"
            })
