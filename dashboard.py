...
# SECTION 1: INITIAL INVESTMENT (Fixed Widget State Mapping)
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    ...
    st.caption(f"写真撮影: {fmt(st.session_state.photo)}")
    ...
    st.caption(f"保証会社: {fmt(st.session_state.guar)}")
    guarantee_fee = st.number_input("保証会社費用 (円)", step=1000, key="guar")
    
    st.caption(f"火災保険: {fmt(st.session_state.fire)}")
...
# Summarizing Startup Costs accurately
