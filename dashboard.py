...
def save_to_google_sheets(name, data_dict):
    df = load_db()
    # Filter out internal Streamlit keys and password
...
# Initialize dynamic room keys (up to 5 types)
for i in range(5):
...
# Sync Callbacks for Slider/Number-Input Bidirectionality
...
# 6. UI: SIDEBAR DATABASE CONTROLS
db = load_db()
...
# SECTION 1: INITIAL INVESTMENT (With Comma Live Preview)
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    st.caption(f"現在の入力: {fmt(st.session_state.rent_total)}")
    ...
    st.markdown("---")
    st.caption(f"敷金: {fmt(st.session_state.shikikin)}")
    shikikin = st.number_input("敷金 (円)", step=1000, key="shikikin")
    st.caption(f"礼金: {fmt(st.session_state.reikin)}")
    reikin = st.number_input("礼金 (円)", step=1000, key="reikin")
    st.caption(f"仲介手数料: {fmt(st.session_state.broker_fee)}")
    broker_fee = st.number_input("仲介手数料 (円)", step=1000, key="broker_fee")
    st.caption(f"リフォーム: {fmt(st.session_state.renov)}")
    renovation = st.number_input("リフォーム (円)", step=10000, key="renov")
    st.caption(f"家具家電: {fmt(st.session_state.furn)}")
    furniture_appliances = st.number_input("家具＋家電 (円)", step=10000, key="furn")
    st.caption(f"許可申請: {fmt(st.session_state.license)}")
    license_fee = st.number_input("旅館業許可 (円)", step=1000, key="license")
    st.caption(f"消防工事: {fmt(st.session_state.fire_work)}")
    fire_safety_work = st.number_input("消防設備の工事 (円)", step=1000, key="fire_work")
    st.caption(f"その他: {fmt(st.session_state.other)}")
    other_init = st.number_input("その他予備費 (円)", step=1000, key="other")
    # Remaining smaller items
    st.session_state.photo = st.number_input("写真撮影 (円)", step=1000, key="photo")
    st.session_state.guar = st.number_input("保証会社費用 (円)", step=1000, key="guar")
    st.session_state.fire = st.number_input("火災保険 (円)", step=1000, key="fire")
...
for i in range(num_types):
    with st.sidebar.expander(f"部屋タイプ {i+1}", expanded=True):
        ...
        st.caption(f"ADR: {fmt(st.session_state.get(f'a_{i}', 0))}")
        r_adr = st.number_input("ADR (円)", step=500, key=f"a_{i}")
        st.caption(f"消耗品: {fmt(st.session_state.get(f'cons_{i}', 0))}")
        r_cons = st.number_input("消耗品/泊 (円)", step=10, key=f"cons_{i}")
        st.caption(f"光熱費: {fmt(st.session_state.get(f'u_{i}', 0))}")
...
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    ...
    st.caption(f"固定管理費: {fmt(st.session_state.fixed_costs)}")
...
startup_cost = (prep_rent_cost + st.session_state.shikikin + st.session_state.reikin + st.session_state.broker_fee + st.session_state.photo + renovation + furniture_appliances + st.session_state.guar + st.session_state.fire + license_fee + fire_safety_work + other_init)
...
export_data = {
    "項目": ["物件名", "初期投資合計", "月間想定売上", "月間費用合計", "月間営業利益", "投資回収期間", "想定稼働率", "平均変動費/泊", "貢献利益率"],
    "値": [new_save_name if new_save_name else "未設定", fmt(startup_cost), fmt(total_rev), fmt(monthly_cost), fmt(profit), f"{payback:.1f}ヶ月", f"{target_occ}%", fmt(avg_var_per_night), f"{(1-total_var_rate)*100:.1f}%"]
}
export_df = pd.DataFrame(export_data)
csv_buffer = io.StringIO()
export_df.to_csv(csv_buffer, index=False)
st.sidebar.download_button(label="📥 CSVエクスポート (配布用)", data=csv_buffer.getvalue(), file_name=f"simulation_{new_save_name}.csv", mime="text/csv")
...
with cl:
    ...
    df_i = pd.DataFrame({"項目": ["空家賃","敷金","礼金","仲介料","リフォーム","家具家電","許可","工事","他"],"金額": [prep_rent_cost, st.session_state.shikikin, st.session_state.reikin, st.session_state.broker_fee, renovation, furniture_appliances, license_fee, fire_safety_work, other_init]})
...
# 11. BREAK-EVEN ANALYSIS SECTION

st.subheader("📈 損益分岐点分析 (Break-Even Analysis)")
col_be1, col_be2 = st.columns([1, 2])

with col_be1:
    st.info("**基本ユニット分析 / Unit Analysis**")
    st.metric(label="平均変動費 / 泊", value=fmt(avg_var_per_night), help="[EN] Costs incurred only when a guest stays (cleaning, utilities). [JP] 宿泊時のみ発生する費用（消耗品、光熱費など）。")
    st.metric(label="固定費合計 / 月", value=fmt(fixed_monthly), help="[EN] Monthly costs regardless of occupancy (rent, basic fees). [JP] 稼働に関わらず毎月支払う費用（家賃、固定管理費）。")
    cm_ratio = (1 - total_var_rate) * 100
    st.metric(label="貢献利益率", value=f"{cm_ratio:.1f}%", help="[EN] Portion of sales to cover fixed costs after OTA/Mgmt fees. [JP] 売上のうち手数料等を除いた、固定費支払いに回せる利益の割合。")

with col_be2:
    st.write("**稼働率別：損益分岐ADR / Required ADR by Occupancy**")
    be_rows = []
    for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
        ...
        if occ_nights > 0:
            ...
            status = "⚠️ 要注意" if be_adr > 18000 else "✅ 健全"
            be_rows.append({"稼働率": f"{occ_p}%", "最低必要ADR": be_adr, "ステータス": status})
    be_df = pd.DataFrame(be_rows)
    st.table(be_df.style.format({"最低必要ADR": "¥{:,.0f}"}))
...
for occ_p in [30, 40, 50, 60, 70, 80, 90, 100]:
    ...
    o_p = o_r - o_c
    analysis_rows.append({"稼働率": f"{occ_p}%", "ADR": o_r/(num_rooms*o_d) if num_rooms*o_d>0 else 0, "売上": o_r, "利益": o_p, "利益率": f"{(o_p/o_r*100):.1f}%" if o_r>0 else "0%", "回収": f"{startup_cost/o_p:.1f}ヶ月" if o_p > 0 else "不可"})
