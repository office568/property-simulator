...
# SECTION 1: INITIAL INVESTMENT (Comma Format Enabled)
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    # Removing format="%d" and using integer step allows commas in modern Streamlit
...
# SECTION 2: ROOM CONFIG (Comma Format Enabled)
...
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    ...
    target_occ_num = c_occ2.number_input("N_Occ", 10.0, 100.0, step=0.1, format="%.1f", key="val_occ_num", on_change=update_occ_slider, label_visibility="collapsed")
    st.write("OTA手数料率 %")
    ...
    ota_fee_num = c_ota2.number_input("N_OTA", 0.0, 30.0, step=0.1, format="%.1f", key="val_ota_num", on_change=update_ota_slider, label_visibility="collapsed")
    st.write("管理費用 %")
    ...
    mgmt_fee_num = c_mgmt2.number_input("N_Mgmt", 0.0, 40.0, step=0.5, format="%.1f", key="val_mgmt_num", on_change=update_mgmt_slider, label_visibility="collapsed")
    ...
    cape_num = c_cape2.number_input("N_Cape", 0.0, 10.0, step=0.5, format="%.1f", key="val_cape_num", on_change=update_cape_slider, label_visibility="collapsed")
