...
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    ...
    guar_fee = st.number_input("保険会社費用 (円)", min_value=0, step=1000, key="guar")
    renovation = st.number_input("リフォーム (円)", min_value=0, step=10000, key="renov")
    furn = st.number_input("家具＋家電 (円)", min_value=0, step=10000, key="furn")
    license_fee = st.number_input("旅館業許可 (円)", min_value=0, step=1000, key="license")
    fire_work = st.number_input("消防設備の工事 (円)", min_value=0, step=1000, key="fire_work")
...
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    ...
    ota_fee = st.number_input("OTA手数料 %", min_value=0.0, step=0.1, key="val_ota")
    mgmt_fee = st.number_input("管理費 %", min_value=0.0, step=0.5, key="val_mgmt")
    fixed_op = st.number_input("固定運営費 (円)", min_value=0, step=1000, key="fixed_costs")
    capex_r = st.number_input("メンテ (CAPEX) %", min_value=0.0, step=0.5, key="val_cape")
...
total_var = sum((r['cons'] + r['util']) * r['count'] * days * occ_rate for r in room_configs)
...
with cl:
    ...
    df_i = pd.DataFrame({"項目": ["空家賃","敷金","礼金","仲介料","保険","リフォーム","家具家電","許可","工事","他"],"金額": [prep_rent_cost, shikikin, reikin, broker_fee, guar_fee, renovation, furn, license_fee, fire_work, other_init]})
...
with cr:
    ...
    df_m = pd.DataFrame({"項目": ["家賃", "変動費", "固定費", "手数料", "利益"],"金額": [rent_total, total_var, fixed_op, commissions, max(0, profit)]})
...
with col_info:
    ...
    st.metric("Total Monthly Fixed Costs", fmt(fixed_total), help="Rent + Fixed Op Costs")
    st.metric("Current Projected Revenue", fmt(total_rev), help="Based on your current ADR and Occupancy settings")
    cm_ratio = (1 - fee_total_rate) * 100
    st.metric("Contribution Margin Ratio", f"{int(round(cm_ratio))}%", help="Revenue left after commissions to pay for fixed costs")

with col_table:
    st.write("**Occupancy vs. Required ADR (Breakeven & Target)**")
    be_data = []
    tot_rooms = sum(r['count'] for r in room_configs)
    fee_denom = (1 - fee_total_rate)
    
    # Calculate for 10% to 100%
