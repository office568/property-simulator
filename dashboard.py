...
with st.sidebar.expander("1. 初期費用・準備期間設定", expanded=True):
    ...
    # Slider + Exact Input for Prep Months
    st.write("開業準備期間 (ヶ月)")
    c_prep1, c_prep2 = st.columns([2, 1])
    prep_months_val = c_prep2.number_input("Exact", value=2, min_value=0, max_value=6, step=1, label_visibility="collapsed", key="exact_prep")
    prep_months = c_prep1.slider("Slider", 0, 6, prep_months_val, label_visibility="collapsed", key="slider_prep")
...
with st.sidebar.expander("3. 運営コスト・稼働率設定", expanded=True):
    # Occupancy Sync (Step changed to 0.1)
    st.write("全体の想定稼働率 %")
    c_occ1, c_occ2 = st.columns([2, 1])
    target_occ_val = c_occ2.number_input("Exact", value=70.0, min_value=10.0, max_value=100.0, step=0.1, label_visibility="collapsed", key="exact_occ")
    target_occ = c_occ1.slider("Slider", 10.0, 100.0, target_occ_val, step=0.1, label_visibility="collapsed", key="slider_occ")

    # OTA Fee Sync (Step changed to 0.1)
    st.write("OTA手数料率 %")
    c_ota1, c_ota2 = st.columns([2, 1])
    ota_fee_val = c_ota2.number_input("Exact", value=15.0, min_value=0.0, max_value=30.0, step=0.1, label_visibility="collapsed", key="exact_ota")
    ota_fee_rate = c_ota1.slider("Slider", 0.0, 30.0, ota_fee_val, step=0.1, label_visibility="collapsed", key="slider_ota")

    # Management Fee Sync
    st.write("管理費用 %")
    c_mgmt1, c_mgmt2 = st.columns([2, 1])
    mgmt_fee_val = c_mgmt2.number_input("Exact", value=20.0, min_value=0.0, max_value=40.0, step=0.5, label_visibility="collapsed", key="exact_mgmt")
    management_fee_rate = c_mgmt1.slider("Slider", 0.0, 40.0, mgmt_fee_val, step=0.5, label_visibility="collapsed", key="slider_mgmt")
    ...
    cape_val = c_cape2.number_input("Exact", value=3.0, min_value=0.0, max_value=10.0, step=0.5, label_visibility="collapsed", key="exact_cape")
    cape_rate = c_cape1.slider("Slider", 0.0, 10.0, cape_val, step=0.5, label_visibility="collapsed", key="slider_cape")
...
for o in [30, 40, 50, 60, 70, 80, 90, 100]:
    ...
    sens_data.append({
        "稼働率": f"{o}%", "ADR": o_adr, "RevPAR": o_revpar, "GOPPAR": o_goppar,
        "売上": o_rev, "費用合計": o_total_cost, "営業利益": o_prof, 
        "利益率": f"{o_margin:.1f}%", "回収期間": f"{total_investment/o_prof:.1f}ヶ月" if o_prof > 0 else "回収不可"
    })

st.table(pd.DataFrame(sens_data).style.format({
    "ADR": "¥{:,.0f}", "RevPAR": "¥{:,.0f}", "GOPPAR": "¥{:,.0f}",
    "売上": "¥{:,.0f}", "費用合計": "¥{:,.0f}", "営業利益": "¥{:,.0f}"
}))
