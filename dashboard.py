...
def save_property_to_db(name, data_dict):
    df = load_db()
    clean_dict = {k: v for k, v in data_dict.items() if not k.startswith("FormSubmit") and "password" not in k and not k.startswith("confirm")}
...
defaults = {
    ...
    "confirm_delete": False # Track deletion state
}
...
if not db.empty:
    ...
    if st.session_state.confirm_delete:
        ...
        if c1.button("✅ はい"):
            ...
            # Reset view to defaults after delete
