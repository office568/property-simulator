...
def load_db(user_email):
    try:
        ...
        if 'owner_email' in df.columns:
            # Filters so users only see their own data
...
def save_to_google_sheets(name, user_email, data_dict):
    ...
    # Clean up internal Streamlit keys
    ...
    if not full_df.empty:
        # Prevent overwriting properties of the same name from DIFFERENT users
...
# --- 5. SMART LOGIN & REDIRECT HANDLING ---
def check_user():
    # Use your specific passcode
    MASTER_CODE = "seirai@2026"
    
    # Check if the website passed ?email= in the URL
    query_params = st.query_params
    if "email" in query_params and "user_email" not in st.session_state:
        st.session_state["user_email"] = query_params["email"].lower()

    if "user_email" not in st.session_state:
        st.title("🏡 物件収支シミュレーター")
        st.info("フォームに登録したメールアドレスとパスコードを入力してください。")
        ...
        st.markdown("---")
        st.caption("※まだ登録がお済みでない方は、公式サイトのフォームよりお申し込みください。")
        return False
    return True
...
st.sidebar.markdown("---")
st.sidebar.markdown("### ☁️ マイ物件 (My Saved Properties)")

if db.empty:
    st.sidebar.info("保存された物件データはありません。")
else:
...
with col_be1:
    ...
    st.metric(label="平均変動費 / 泊", value=fmt(round(avg_var_per_night)), help="宿泊時のみ発生する実費（リネン、光熱費等）。")
    st.metric(label="固定費合計 / 月", value=fmt(round(fixed_monthly)), help="家賃、インターネット等の毎月固定の支払。")
