...
# initial_sidebar_state="expanded" ensures the sidebar is open on first load
st.set_page_config(
...
# 3. GOOGLE SHEETS CONNECTION (Multi-Tenant Logic)
...
def save_to_google_sheets(name, user_email, data_dict):
    ...
    if full_df is not None and not full_df.empty:
        if 'property_name' in full_df.columns and 'owner_email' in full_df.columns:
            # Prevents users from overwriting properties of the same name from DIFFERENT users
...
# --- 5. SECURE LOGIN & SECURITY FOOTER ---
def check_user():
    MASTER_CODE = "seirai@2026"
    query_params = st.query_params
    
    # Auto-Login via URL Redirect (?email=...)
    ...
    if "user_email" not in st.session_state:
        ...
        with st.container():
            ...
            with sec_col1:
                st.markdown("**🔒 データの暗号化**\n通信はSSL/TLSで保護され、データはGoogle Cloud基準で暗号化保存されます。")
                st.markdown("**👤 プライバシー分離**\nデータは各ユーザーごとに厳格に分離され、他者に公開されることはありません。")
            with sec_col2:
                st.markdown("**🔑 認証システム**\n個別のパスコード認証により、第三者による不正アクセスを防止します。")
                st.markdown("**📋 非公開の原則**\n保存データはサポート目的以外で使用されることはありません。")
        
        st.caption("※未登録の方は、公式サイトのフォームよりお申し込みください。")
        return False
    return True
...
with col_be1:
    ...
    st.metric(label="貢献利益率", value=f"{round(cm_ratio)}%", help="売上100円につき、手数料などを払った後に家賃等の固定費の支払いに回せる利益の割合です。")
