...
# Pre-initialize room keys to avoid TypeErrors
for i in range(5):
    if f"name_{i}" not in st.session_state: st.session_state[f"name_{i}"] = f"タイプ {chr(65+i)}"
    if f"c_{i}" not in st.session_state: st.session_state[f"c_{i}"] = 1
    if f"a_{i}" not in st.session_state: st.session_state[f"a_{i}"] = 0
    if f"cons_{i}" not in st.session_state: st.session_state[f"cons_{i}"] = 0
    if f"u_{i}" not in st.session_state: st.session_state[f"u_{i}"] = 0
...
# 6. UI: LOGO & DATABASE (TOP OF SIDEBAR)
db = load_db()
try:
...
if not db.empty:
    ...
    if st.sidebar.button("選択した物件を読み込む"):
        ...
        # SANITIZATION: Force types to prevent TypeErrors
        for key, value in saved_row.items():
            if key == "property_name": continue
            
            # If value is NaN (empty in CSV), replace with default
            if pd.isna(value):
                st.session_state[key] = defaults.get(key, 0)
            # Force Strings for room names
            elif "name_" in key:
                st.session_state[key] = str(value)
            # Force Floats for percentages/sliders
            elif "val_" in key or "rate" in key:
                st.session_state[key] = float(value)
            # Force Integers for counts and yen amounts
            else:
                try:
...
num_types = int(st.sidebar.number_input("部屋タイプの種類数", min_value=1, max_value=5, key="num_types"))
room_configs = []
for i in range(num_types):
    with st.sidebar.expander(f"部屋タイプ {i+1}", expanded=True):
        r_name = st.text_input(f"名 {i+1}", key=f"name_{i}")
        r_count = st.number_input(f"数 {i+1}", min_value=1, key=f"c_{i}")
        r_adr = st.number_input(f"ADR {i+1}", key=f"a_{i}")
        r_cons = st.number_input(f"消耗品 {i+1}", key=f"cons_{i}")
        r_util = st.number_input(f"光熱費 {i+1}", key=f"u_{i}")
...
if st.sidebar.button("💾 この設定を保存する"):
    ...
    save_property_to_db(new_save_name, to_save)
